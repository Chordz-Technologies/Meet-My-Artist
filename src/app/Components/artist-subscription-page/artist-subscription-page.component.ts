      ...
      if (orderResponse.status === 'success') {
        ...
        if (paymentResponse) {
          ...
          if (verificationResponse.status === 'success') {
            ...
            // Update artist status to 'Active' and update subscription date if userType is 'organizer'
            const userId = parseInt(localStorage.getItem('userId') || '0', 10);
            const newSubsDate = new Date().toISOString().split('T')[0]; // Current date in YYYY-MM-DD format
            if (userId && this.userType === 'artist') {
              const updateResponse = await this.service
                .updateArtistStatus(userId, 'Active', newSubsDate)
                .toPromise();
              if (updateResponse.status === 'success') {
                this.toastr.success('Artist status updated to active!');
              } else {
                this.toastr.error('Failed to update artist status.');
              }
            }
